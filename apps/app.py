...
def credit_risk_prediction(input_data):
    ...
    input_data_as_dataframe["cb_person_default_on_file"] = (
        loaded_label_encoder_model.transform(
            input_data_as_dataframe["cb_person_default_on_file"]
        )
    )
    ...
    one_encoded_data = loaded_one_hot_encoder.transform(
        input_data_as_dataframe[category_cols]
    )
    one_encoded_df = pd.DataFrame(
        one_encoded_data,
        columns=loaded_one_hot_encoder.get_feature_names_out(category_cols),
    )
    ...
    final_X = final_X[loaded_model_columns]
...
def main():
    ...
    with col1:
        ...
        PersonAge = st.number_input(
            "Enter Applicant Age", min_value=18, max_value=110, step=1
        )
        PersonIncome = st.number_input(
            "Enter Annual Income ($)", min_value=0, step=1000
        )
        PersonEmpLength = st.number_input(
            "Enter Employment Length (Years)", min_value=0.0, max_value=65.0, step=0.5
        )
        CbPersonCreditHistoryLength = st.number_input(
            "Enter Credit History Length (Years)", min_value=0, max_value=50
        )
    ...
    with col2:
        ...
        LoanAmount = st.number_input(
            "Enter Requested Loan Amount ($)", min_value=0, step=500
        )
        LoanInterestRate = st.number_input(
            "Enter Interest Rate (%)", min_value=0.0, max_value=30.0, step=0.1
        )
        LoanPercentIncome = st.number_input(
            "Enter Loan Percent Income (0.0 - 1.0)",
            min_value=0.0,
            max_value=1.0,
            step=0.01,
        )
        ...
        with c1:
            PersonHomeOwnership = st.selectbox(
                "Home Ownership", ["RENT", "OWN", "MORTGAGE", "OTHER"]
            )

        with c2:
            LoanIntent = st.selectbox(
                "Loan Purpose",
                [
                    "PERSONAL",
                    "EDUCATION",
        ...
        with c4:
            CbPersonDefaultOnFile = st.selectbox(
                "Previous Default History?", ["y", "n"]
            )
